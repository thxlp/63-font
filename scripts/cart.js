...
async function saveCartHistory() {
    ...
    try {
        ...
        const response = await fetch('https://heath-check-436133641220.asia-southeast1.run.app/api/cart/save', {
            method: 'POST',
            headers: {
    ...
    if (userId) {
        try {
            ...
            const response = await fetch(
                `https://heath-check-436133641220.asia-southeast1.run.app/api/cart/history?user_id=${encodedUserId}&limit=${limit}&offset=${offset}`
            );
...
async function deleteCartHistory(historyId) {
    ...
    if (userId && !isNaN(parseInt(historyId))) {
        try {
            const response = await fetch(`https://heath-check-436133641220.asia-southeast1.run.app/api/cart/${historyId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                },
...
async function saveDailyCalorieLog() {
    try {
        ...
        const apiEndpoints = [
            'https://heath-check-436133641220.asia-southeast1.run.app/api/data/calorie_logs',
            'https://heath-check-436133641220.asia-southeast1.run.app/api/calorie_logs',
            'https://heath-check-436133641220.asia-southeast1.run.app/api/data/daily_logs'
        ];
...
async function getUserTargetCalories() {
    try {
        ...
        // ลองดึงจาก API ก่อน
        const apiEndpoints = [
            `https://heath-check-436133641220.asia-southeast1.run.app/api/data/bmi_records?user_id=${userId}`,
            `https://heath-check-436133641220.asia-southeast1.run.app/api/data/bmi_records`,
        ];
